...
7. Saves pairplots of the shared MDS embedding:
      - color-coded by Clinician/Leah/Gemini/GPT
      - color-coded by response-length bins
...
def save_pairplot_by_length(
    projected_df: pd.DataFrame,
    dims_to_plot: List[str],
    outpath: str | Path,
) -> None:
    """
    Pairplot of all shown MDS dimensions colored by response-length bins.
    Bins: [0-20], [20-100], [100-200], [200-500], [500-1000]
    """
    ...
    sns.set_context("talk")
    palette = dict(zip(labels, sns.color_palette("viridis", n_colors=len(labels))))

    cols = dims_to_plot + ["len_bin"]
    ...
    g = sns.pairplot(
        ...
        diag_kind="hist",
        corner=True,
        plot_kws={"alpha": 0.7, "s": 28, "linewidth": 0},
    )
    g.figure.suptitle("Joint MDS pairplot colored by response length", y=1.02)

    if g._legend is not None:
        g._legend.set_title("Length bin")
        for text in g._legend.texts:
            text.set_fontsize(9)
...
def main():
    ...
    print("Saving joint MDS pairplots...")
    ...
    save_pairplot_by_group(
        projected_df=projected_df,
        dims_to_plot=mds_cols,
        outpath=outdir / f"joint_pairplot_mds_k{optimal_k}_showing_{plot_k}dims.png",
    )

    save_pairplot_by_length(
        projected_df=projected_df,
        dims_to_plot=mds_cols,
        outpath=outdir / f"joint_pairplot_length_binned_k{optimal_k}_showing_{plot_k}dims.png",
    )
